...
const increase = () => {
    ...
    if(data.len-1<=data.curnum){
        ...
        data.curnum+=1;
        data.problem = questions.value[data.curnum].number +'、'+ questions.value[data.curnum].text;
        options.splice(0, options.length);
        for(let i=0; i<option.value[data.curnum].length; i++){
            options.push({
                label: option.value[data.curnum][i],
                value: (i + 1).toString(),
            });
...
onMounted(() => {
    ...
    data.problem = questions.value[0].number +'、'+ questions.value[0].text;
    store.curtopic = data.problem;
    console.log(option.value[0])
    const myArray = [];
for (const key in option.value[0]) {
  if (option.value[0].hasOwnProperty(key)) {
    myArray.push(option.value[0][key]);
  }
}
    console.log(typeof myArray); 
    store.setCurOptions(option.value[0] || []);
    for(let i=0; i<option.value[0].length; i++){
        options[i].label = option.value[0][i];
        options[i].value = i+1+'';
        let l = option.value[0][i];
    }
})
