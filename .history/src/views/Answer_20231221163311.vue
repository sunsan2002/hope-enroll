...
const decrease = () => {
    ...
    if(data.curnum<=0){
        ...
        store.curoptions.push(option.value[data.curnum]);
        options.splice(0, options.length);
        let str = option.value[data.curnum]; //选项
        const matches = str.match(/[A-Z]、[^A-Z]+/g); //存数组
        store.setOptions(matches || []);
        cnt2++;
        if(cnt2===1){
            store.setCurOptions(matches || []);
            // store.curoptions.push(matches || []);
        }
        // for(let i=0; i<option.value[data.curnum].length; i++){
        //     options.push({
        //         label: option.value[data.curnum][i],
...
onMounted(() => {
    ...
    data.problem = store.curtopic;
    console.log(store.curtopic)
    console.log(store.curoptions)
