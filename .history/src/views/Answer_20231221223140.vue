...
const conversion = () =>{
    ...
    console.log(data.curnum+" "+x)
    let now = data.curnum
    const existingIndex = store.ans.findIndex(item => item.number === data.curnum+1);
    if (existingIndex !== -1) {
        // 如果存在，则替换text的值
...
//A->1
const reversal = () =>{
    const existingIndex = store.ans.findIndex(item => item.number === data.curnum);
    if (existingIndex !== -1) {
        // 如果存在
        if(store.ans[data.curnum].text === ''){
            radio.value='';
        }else if(store.ans[data.curnum].text === 'A'){
            radio.value = '1';
        }else if(store.ans[data.curnum].text === 'B'){
            radio.value = '2';
        }else if(store.ans[data.curnum].text ==='C'){
            radio.value = '3';
        }else if(store.ans[data.curnum].text ==='D'){
            radio.value = '4';
        }else if(store.ans[data.curnum].text ==='E'){
            radio.value = '5';
        }
    } else {
        // 如果不存在
