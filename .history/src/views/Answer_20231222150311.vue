...
const conversion = () =>{
    ...
    console.log("当前是第"+now+"题,存在吗"+existingIndex)
    if (existingIndex !== -1) {
        // 如果存在，则替换text的值
        store.ans[now].text = x;
    } else {
        // 如果不存在，则添加新的键值对
        store.setAns({
            number: data.curnum,
...
const reversal = (now:number) =>{
    ...
    if (existingIndex !== -1) {
        // 如果存在
        console.log("这是什么"+store.ans[data.curnum].text)
    ...
    console.log("下一题的选项"+radio.value)
}
