    ...
    <div class="containe">
        <el-card class="box-card">
            ...
            <div class="card-footer">
                <el-button type="primary" key="primary" text bg size="large" class="button" @click="finish()">我答完了</el-button>
            </div>
        </el-card>
...
const finish = () =>{
    ...
    for(let i=0; i<store.ans.length; i++){
        // console.log(store.ans[i].text);
        answer += store.ans[i].text;
    }
    const number: string | undefined = this.$route.params.num;
    apiFun.user.submit({
        select: this.$route.params.num,
        answer: answer,
    })
    .then((res: any) => {
    console.log("存储答案信息:")
...
const reversal1 = (now:number) =>{
    ...
    let ans = radio.value;
    let nnow = now;  //+1回退最后一个正常存,但是再向后一个data.curnum会没有+1    /////bug
