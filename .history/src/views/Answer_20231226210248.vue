...
const increase = () => {
    if(data.len-1<=data.curnum){
        ...
        console.log("(下一个)题目序号1:"+next+" 当前："+store.curstate)
        reversal(next);
        data.curnum+=1;
...
const reversal = (now:number) =>{
    let nnow = now;
    let existingIndex = store.ans.findIndex(item => item.number === now);
    if(store.curstate === -1){
        nnow+=1;
        // store.curnum+=2;
        existingIndex = store.ans.findIndex(item => item.number === nnow);
    }
    console.log("下一个题的序号"+nnow+",存在吗"+existingIndex)
    if (existingIndex !== -1) {
        // 如果存在
