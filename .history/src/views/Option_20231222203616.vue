    ...
    for(let i=1; i<content.value.length; i++){
        ...
        if(flag === 0){
            if(typeof content.value[i][0] === 'number'){
                ...
                const result1 = str.substring(0, indexA);
                const result2 = str.substring(indexA);
                console.log(num+'、'+ result2);
                //题目
                store.setQuestion({
                    number: num,
                    text: result1,
                })
                const strWithoutComma = result2.replace(/，/g, '  ');
                let res: string = str.replace(/([A-Z])+/g, (match: string) => {
                    return match.replace(/([A-Z])/g, '$1、');
                });
                console.log(strWithoutComma);
                const optionsArray = strWithoutComma.match(/([A-Z]、[^A-Z]*)/g);
                console.log(optionsArray);
                //选项
                // const matches = result2.match(/([A-Z]、[^A-Z]+)/g); //存数组
                // console.log(matches);
                // store.setOptions(matches || []);
            }
