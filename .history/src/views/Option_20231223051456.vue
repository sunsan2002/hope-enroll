...
const text4 = (content:any) =>{
    for(let i=1; i<content.value.length; i++){
        ...
        if(isFirstCharNumeric){
            ...
            let num = content.value[i][0][0]; //题目编号
            var index = content.value[i][0].indexOf("．");  // 查找点的位置
var afterDot = index !== -1 ? content.value[i][0].substring(index + 1) : str;
            let str =  content.value[i][0].split(/./)[1]; //题干
            console.log(num+'、'+ str);
            store.setQuestion({
                number: num,
                text: str,
            ...
            if(content.value[i][0][0] === 'A'){
                let str = content.value[i][0]; //选项
                // console.log(str);
                const matches = str.match(/[AA-Z]、[^A-Z]+/g); //存数组
                store.setOptions(matches || []);
            }
        }    
    }
