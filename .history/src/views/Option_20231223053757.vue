    ...
    for(let i=1; i<content.value.length; i++){
        ...
        if(isFirstCharNumeric){
            ...
            if(content.value[i][0][0] === 'A'){
                ...
                console.log(matches)
                // 给定字符串
const str1 = "A．继续读书或看杂志，或继续看电影，不太注意正在发生的骚乱 B．注意事态的变化，仔细听播音员的播音，并翻看紧急情况应付手，以备万一 C．A和B都有一点 D．不能确定--根本没注意到";

// 使用正则表达式提取选项和选项内容
const pattern = /([A-D])．(.*?)\s*(?=[A-D．]|$)/g;
const matches1 = Array.from(str.matchAll(pattern));

// 将结果存储到数组中
const optionsAndContents = matches.map(match => `${match[1}. ${match[2]}`);

// 打印结果
console.log(optionsAndContents);
            }
        }    
    }
}
