    ...
    for(let i=1; i<content.value.length; i++){
        ...
        if(flag === 0){
            if(typeof content.value[i][0] === 'number'){
                ...
                //选项
                let strWithoutComma = result2.replace(/[.，]/g, '');
                let res: string = strWithoutComma.replace(/([A-Z])+/g, (match: string) => {
            ...
            if(typeof content.value[i][0] === 'number'){
                let result1=''
                if(content.value[i][0][0]==='二'){
                    result1 = '在下列每一对词语中，哪一个词语更合你心意？请仔细想想这些词语的意义，而不要理会他们的字形或读音。'
                }else{
                    result1 = '在下列每一对词语中，哪一个词语更合你心意？'
                }
                //题目
                store.setQuestion({
                    number: num,
                ...
                //选项
                let res: string = str.replace(/([A-Z])+/g, (match: string) => {
                    return match.replace(/([A-Z])/g, '$1、');
                });
...
const text4 = (content:any) =>{
    for(let i=1; i<content.value.length; i++){
        ...
        if(isFirstCharNumeric){
            ...
            if(content.value[i][0][0] === 'A'){
                let str = content.value[i][0]; //选项
                // let strWithoutComma = str.replace(/[．]/g, '');
                // let res: string = strWithoutComma.replace(/([A-Z])+/g, (match: string) => {
                //     return match.replace(/([A-Z])/g, '$1、');
                // });
                // let matches = res.match(/([A-Z]、[^A-Z]*)/g); //存数组
                // let matches = str.match(/([A-D])．(.*?)\s*(?=[A-D．]|$)/g);
// 给定字符串
const str1: string = "A．继续读书或看杂志，或继续看电影，不太注意正在发生的骚乱 B．注意事态的变化，仔细听播音员的播音，并翻看紧急情况应付手，以备万一 C．A和B都有一点 D．不能确定--根本没注意到";

// 使用正则表达式提取选项和选项内容
const pattern: RegExp = /([A-D])．([\s\S]*?)(?=[A-D．]|$)/g;
const matches: RegExpMatchArray | null = str1.match(pattern);

// 将结果存储到数组中
const optionsAndContents: string[] = matches ? Array.from(matches).map(match => `${match[1]}. ${match[2].trim()}`) : [];

// 打印结果
optionsAndContents.forEach(item => console.log(item));
                // store.setOptions(matches || []);
                // console.log(matches)
