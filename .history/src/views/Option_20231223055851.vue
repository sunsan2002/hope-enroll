    ...
    for(let i=1; i<content.value.length; i++){
        ...
        if(isFirstCharNumeric){
            ...
            if(content.value[i][0][0] === 'A'){
                ...
                // });
                // let matches = res.match(/([A-Z]、[^A-Z]*)/g); //存数组
                // let matches = str.match(/([A-D])．([\s\S]*?)(?=[A-D．]|$)/g);
                // store.setOptions(matches || []);
                // console.log(matches)
                // 使用正则表达式提取选项和选项内容
                const pattern: RegExp = /([A-D])．(.*?)\s*(?=[A-D．]|$)/g;
                const matches: RegExpMatchArray | null = str.matchAll(pattern);

                // 将结果存储到数组中
                const optionsAndContents: { option: string, content: string }[] = matches ? Array.from(matches).map(match => ({ option: match[1], content: match[2] })) : [];
